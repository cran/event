  ...
  for(i=0;i<*nind;i++){
	...
	if(!*density){
	  ...
	  case 3:
	    ...
	    l1=dgamma(yy,lambda,bt)/(1-pgamma(yy,lambda,bt));
	    break;
	  case 4:
	  ...
	  case 5:
	    ...
	    l1=dnorm(ly,bt,lambda)/yy/(1-pnorm(ly,bt,lambda));
	    break;
	  case 6:
	    H=ihlogis(ly,bt,lambda);
	    l1=dlogis(ly,bt,lambda)/yy/(1-plogis(ly,bt,lambda));
	    break;
	  case 7:
	    H=-log(1-pcauchy(ly,bt,lambda));
	    l1=dcauchy(ly,bt,lambda)/yy/(1-pcauchy(ly,bt,lambda));
	    break;
	  case 8:
	  ...
	  case 2:
	    ...
	    l1=dweibull(yy,lambda,bt);
	    break;
	  case 3:
	    H=pgamma(yy,lambda,bt);
	    l1=dgamma(yy,lambda,bt);
	    break;
	  case 4:
	  ...
	  case 5:
	    ...
	    l1=dnorm(ly,bt,lambda)/yy;
	    break;
	  case 6:
	    H=plogis(ly,bt,lambda);
	    l1=dlogis(ly,bt,lambda)/yy;
	    break;
	  case 7:
	    H=pcauchy(ly,bt,lambda);
	    l1=dcauchy(ly,bt,lambda)/yy;
	    break;
	  case 8:
	    tmp=exp(-fabs(ly-bt)/lambda)/2;
	    H=ly<bt?tmp:1-tmp;
	...
	if(*dist==2){
	  if(c)*like-=c*log(l1)-b*H-lgamma(a)+log(b)+(a-1)*log(b*H);
	  /*	if(c)*like-=c*log(l1)+log(dgamma(H,a,1/b));*/
	  else *like+=ihgamma(H,a,1/b);} /* -=log(1-pgamma(H,a,1/b));} */
	else if(*dist==3){
	  if(c)*like-=c*log(l1)-pow(b*H,a)+(a-1)*log(b*H)+log(b)+log(a);
	  else *like+=pow(b*H,a);}   /* -=log(1-pweibull(H,a,1/b));} */
